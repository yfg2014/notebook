...
EOF;
    ...
    var $command_backup_shopex_id = "备份shopex_id";
    var $command_backup_shopex_id_options = array(
        'backup_dir'=>array('title'=>'备份文件存放目录','short'=>'d'),
        'name'=>array('title'=>'要备份的项目名称','short'=>'n'),    
    );
    function command_backup_shopex_id(){
        ...
        if($options['backup_dir']){
            ...
            if(!$fp = fopen($backup_file, 'w')){ echo "Can not find file......"; exit;}
    ...
    var $command_recover_shopex_id = "恢复 shopex_id";
    var $command_recover_shopex_id_options = array(
        'backup_dir'=>array('title'=>'备份文件存放目录','short'=>'d'),
        'name'=>array('title'=>'要备份的项目名称','short'=>'n'),    
    );
    ...
    function command_recover_shopex_id(){
        ...
        if($options['backup_dir']){
            ...
            if(is_file($backup_file)){
                if(!($fp = fopen($backup_file,'r'))){ echo "Failed to read file";exit;};
                $content = fgets($fp);
                ...
                if(!$result){
                    echo "failed shopex_id recovery.....";exit;
                }else{
    ...
    var $command_backup_certificate_id = "备份证书号";
    var $command_backup_certificate_id_options = array(
        'backup_dir'=>array('title'=>'备份文件存放目录','short'=>'d'),
        'name'=>array('title'=>'要备份的项目名称','short'=>'n'),    
    );
    ...
    var $command_recover_certificate_id = "恢复证书号";
    var $command_recover_certificate_id_options = array(
        'backup_dir'=>array('title'=>'备份文件存放目录','short'=>'d'),
        'name'=>array('title'=>'要备份的项目名称','short'=>'n'),    
    );
