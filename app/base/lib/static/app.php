        ...
        if(!isset($this->__appConf[$key])){
            if(base_kvstore::instance('setting/'.$this->app_id)->fetch($key, $val) === false){
                if(!isset($this->__appSetting[$this->app_dir])){
                    if(defined('CUSTOM_CORE_DIR') && file_exists(CUSTOM_CORE_DIR.'/'.$this->app_id.'/setting.php')){
                        $setingDir = CUSTOM_CORE_DIR.'/'.$this->app_id.'/setting.php';
                    }else{
