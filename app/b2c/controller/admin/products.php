    ...
    function _set_spec($spec){
        ...
        foreach($specinfo as $k=>$v){
            foreach($v['option'] as $k2=>$v2){
                ...
                $tmp_spec_goods_img[$v2['private_spec_value_id']] = $v2['spec_goods_images']&&$v2['spec_goods_images']!='null'?$v2['spec_goods_images']:$default_spec_image;
...
//        $this->pagedata['spec_default_pic'] = $this->app->getConf('spec.default.pic');
    ...
    function addSpecValue(){
        ...
        foreach($aTmp as $k=>$v){
            $goods_spec_images[] = $v&&$v!=''?$v:$this->pagedata['spec_default_pic'];
